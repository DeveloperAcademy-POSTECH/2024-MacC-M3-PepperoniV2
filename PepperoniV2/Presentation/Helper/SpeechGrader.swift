...
func calculateIntonation(referenceFileName: String, comparisonFileURL: URL) -> Double {
    ...
    let m4aFileName = referenceFileName.replacingOccurrences(of: ".mov", with: ".m4a")
    
    // Bundle에서 m4a 파일의 URL 가져오기
    guard let path = Bundle.main.path(forResource: m4aFileName, ofType: "m4a") else {
        print("참조 파일(m4a)을 찾을 수 없습니다: \(m4aFileName)")
        return 0.0
    }

    let referenceURL = URL(fileURLWithPath: path)

    print("참조 파일 URL: \(referenceURL)") // 디버깅용
